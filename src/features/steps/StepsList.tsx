...
const STEPS: VotingGuideStep[] = [
  ...
  },
  {
    ...
    icon: '✊',
    aiQuestion: 'What happens after voting day in India, including counting and government formation?',
  },
];
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <label className="text-sm">
                Indian citizen?
                <select value={citizenship} onChange={(e) => setCitizenship(e.target.value)} className="input-base mt-1">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={mentallyFit} onChange={(e) => setMentallyFit(e.target.checked)} />
              Mentally fit to vote
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={servingSentence} onChange={(e) => setServingSentence(e.target.checked)} />
              Currently serving a criminal sentence
            </label>
            {eligibilityResult && <p className="text-sm font-semibold text-navy">{eligibilityResult}</p>}
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm text-slate-700">
            <p><strong>Booth Number:</strong> 124</p>
            <p><strong>Address:</strong> Government Primary School, Ward 8</p>
              ...
              {BOOTH_STAGES.map((stage, index) => (
                <div
                  ...
                  className={cn(
                    ...
                    boothStageIndex === index ? 'border-orange bg-orange-50 animate-pulse' : 'border-slate-200 bg-white'
                  )}
                >
                  {stage}
                </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
            ...
            {showVvpat && selectedCandidate && (
              <div className="mt-3 border border-green-300 bg-green-50 rounded-lg p-3 animate-fade-in">
                <p className="text-sm font-semibold text-green-800">VVPAT Slip: {selectedCandidate}</p>
                <p className="text-xs text-green-700">Visible for 7 seconds. Verify and press CLOSE.</p>
                <button
                  onClick={() => {
                    setShowVvpat(false);
                    setVoteDone(true);
            ...
            {voteDone && <p className="mt-3 text-sm font-semibold text-navy">Your vote is cast successfully.</p>}
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200 rounded-2xl p-4 md:p-5">
        ...
        <div className="h-3 bg-slate-100 rounded-full overflow-hidden mb-3" role="progressbar" aria-valuenow={completionPct} aria-valuemin={0} aria-valuemax={100}>
          <div className="h-full bg-gradient-to-r from-navy to-orange transition-all duration-300" style={{ width: `${completionPct}%` }} />
        </div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active ? 'border-navy bg-navy-50' : 'border-slate-200 bg-white hover:border-navy-300'
                )}
              >
                <div className="flex items-center gap-1 text-xs font-semibold text-slate-700">
                  {completed ? <CheckCircle2 className="w-3.5 h-3.5 text-green-600" /> : <span>{item.number}.</span>}
                  <span>{item.icon}</span>
                </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 md:p-6">
        ...
        <div className="grid lg:grid-cols-[1fr_260px] gap-5">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col justify-between">
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Illustration</p>
